...
- 用户直接发 115 分享链接+密码 → `media_ctl run share115`（自动转存，无需二次确认）。**密码不能是 `***`**；masked password 会直接失败并要求重新解锁/给明文。
- 模糊片名先 `media_ctl run identify` 定 tmdb_id，确认后再 search/watch。
- 用户要找/看影视 → `media_ctl run watch`（内部也会 identify）；不要手搓 MoviePilot JSON。
- **用户说「转存网盘 / 走 115 / HDHive」** → `run hdhive`（grab+transfer），不要先 PT 下载再绕。
- **watch 认片**：仅有 tmdb_id 时会同时尝试 电影/电视剧 detail，避免电影被默认当电视剧导致空 media + PT 搜空。
- **HDHive 路径**：`run hdhive` / `watch --prefer auto|hdhive` 走 grab（解锁+转存）。真实 masked `password=***` 或 transfer `code!=0` 一律算失败；watch 自动回落 PT；只有 unlock+transfer 都成功才跳过 PT。
- **P115 插件挂了**：若 transfer 回 `参数错误` / 插件加载报 `TransferRenameBuild`，先修/升 P115StrmHelper 与 MoviePilot 兼容，再重试 hdhive；不要反复改 share URL 形状装忙。
...
10. **`watch` 报 `no_resources` 但直觉有货** — 先看 media 是否空壳（title/type/tmdb_id 全 null）。电影被当电视剧是旧 bug；应走新 identify（双 type 试探），或显式 `--media-type movie`。不要立刻跳去手搓 search JSON。
11. **网盘转存主路径** — `run hdhive --param tmdbid=... --param media_type=movie|tv --param transfer=true`。不要：只 search 不 grab、把 masked `***` 丢给 share115、把 `code=-1` 当成功。
12. **HDHive 成功定义** — unlock 出明文密码 **且** P115 `code==0`（或「已经转存」）。仅解锁成功不算完成；仅 PT 下载不算网盘转存。
13. **插件不兼容** — MoviePilot 日志 `ChainEventType.TransferRenameBuild` / transfer 恒 `参数错误` → 升/降 P115StrmHelper，不是改 skill URL 编码。
...
1. **identify** — `recognize` / `media/search` / `media/tmdb:{id}`；仅 tmdb_id 时**电影+电视剧双试**，空 shell 不算成功
2. **HDHive（可选）** — 默认 `prefer=auto` 走 grab（解锁+转存）；成功则**直接返回**；失败继续 PT。可用 `--skip-hdhive` / `--prefer pt` / `--hdhive-only`
3. **PT 搜索 fallback 矩阵**
...
成功判据：`success=true` 且 `result.transfer.code==0`（或 msg 含「已经转存」）。
常见落盘：`save_parent.path`（如 `/share/unsorted`）。

细节与排障：`references/hdhive-115.md`。

底层步骤（仅排障）：

1. CDP 必须连 `type=page`，禁止 `service_worker`。
2. `hdhive.py tmdb movie|tv <tmdbid>` → resources → unlock 明文密码。
3. `transfer_share_to_moviepilot(share_url)` / `run share115`；**拒绝** `password=***`。
4. 日志里 `password=***` 可能是脱敏展示；以 transfer `code` 为准，不要把脱敏当失败根因。
...
- P115StrmHelper 必须与 MoviePilot 事件链兼容；加载失败时 API 仍可能回 `参数错误`，先修插件再重试转存。
- identify/watch 对电影禁止默认 `type_name=电视剧`；空 media shell（全 null）= 失败，不是成功。
