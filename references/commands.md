...
成功看 `transfer.code==0` 与 `save_parent.path`。详见 `references/hdhive-115.md`。
...
python3 scripts/hdhive.py unlock "<resource_url>"      # 解锁获取 115 链接（明文密码）
python3 scripts/hdhive_grab.py "关键词" --select N     # 一键搜索→解锁→转存
```

Unlock 三步：点「确定解锁」→ 点确认对话框「确定」→ 点 115 协议确认页「确定」→ 取 `location.href` 明文密码。

自动选资源逻辑：质量偏好 + `(-疑似失效, 官组, 4K, 免费)`。

硬规则：真实 `password=***` 禁止转存；插件 `参数错误`/`TransferRenameBuild` 先修 P115 兼容。
