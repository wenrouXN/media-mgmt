...
| **转存网盘 / 115 找源** | **`run nextfind`** | tmdbid+media_type 或 title；先 dry | resources 空禁盲 grab |
...
python3 scripts/media_ctl.py run nextfind --param tmdbid=ID --param title=片名 --param media_type=movie --param transfer=true
...
| nextfind 网盘失败要本地种 | `run watch --param prefer=pt`（或 skip_nextfind=true） |
| 已有 115 分享转存失败 | 见 `references/nextfind-115.md`，核对明文 password + share115 |
...
| 网盘 / NextFind / share115 | `references/nextfind-115.md` |
| 装机 / 后端矩阵 | `INSTALL.md` |
